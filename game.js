$(window).load(function() {
	...
	var cp				= document.getElementById("player");
		$('#player').css('width', player_size[0]);
		$('#player').attr('width', player_size[0]);
		$('#player').attr('height', player_size[1]);
		$('#player').css('left', ( (canvas_width / 2) /*- (player_size[0] / 2)*/ ) );
		$('#player').css('top', ( (canvas_height / 2) /*- (player_size[1] / 2)*/ ) );
	canvas_player	= ct.getContext("2d");
	
	$('#game_screen').css('width', canvas_width);
	$('#game_screen').css('height', canvas_height);
	
	game_start(); //Start the gamew

});
	var canvas_tiles 	= null;
	var canvas_player 	= null;

	var tile_size 			= new Array(20, 20); 	//defines pixel size per tile
	var player_size			= new Array(20, 20);	//Defines the player sprite size in px
	...
	var game_fps			= 1; 				//FPS for the game to update, lower number slower the game runs but more stable
	...
	function game_start(){
		...
		//Generate test level
		var level_1 = new Level(map_size[0], map_size[1], "Intro land", 50, 10, 10, 8, 8);
		...
		level_1.grid[starter_y][starter_x] = 9;
		
		//Create the player
		var player	= new Player('Testificate', 1);
	...
	//Funciton to controll data changes
	function update(player, world_loader){
		canvas_tiles.clearRect(0, 0, canvas_width, canvas_height);
		
		if( player.limiter_counter > 0 ){ player.limiter_counter --; debug_log(player.limiter_counter); }
		if( player.move() ){
	...
	//Function to controll drawing on the canvas
	function draw(player, world_loader, level_1){
		world_loader.draw(canvas_tiles);
		//draw_map(level_1);
		//player.draw(canvas_player);
		//game_running	= false;
	}
	...
	function draw_map(level){
		for(var y=0; y<level.grid_size[1]; y++){
			for(var x=0; x<level.grid_size[0]; x++){
				...
				canvas_tiles.fillRect((x*4),(y*4),(x*4+4),(y*4+4));
			}
		}
	}
	...
	function check_cell(pos_x, pos_y){
		...
		if( world_loader.current_level.grid[pos_y][pos_x] == 1 || world_loader.current_level.grid[pos_y][pos_x] == 7 ){
			return true;
		}
		return false;
	}